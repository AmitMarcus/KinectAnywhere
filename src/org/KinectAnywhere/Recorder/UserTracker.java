...
public class UserTracker extends Component
{
    ...
    private final String SAMPLE_XML_FILE = "src\\org\\KinectAnywhere\\Recorder\\SamplesConfig.xml";
    public UserTracker(String filePath, Integer cameraId)
    {
    ...
    public void drawSkeleton(Graphics g, int user) throws StatusException
    {
		...
		// Record skeleton frame to file
		MySkeletonFrame skeletonFrame = new MySkeletonFrame((HashMap<SkeletonJoint, MySkeletonJointPosition>)dict.clone(), this.cameraId);

		try {
			FileOutputStream fos = new FileOutputStream(this.filePath, true);
			ObjectOutputStream oos = new ObjectOutputStream(fos);
			oos.writeObject(skeletonFrame);
			oos.close();
		}catch (IOException e) {
